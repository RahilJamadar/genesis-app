  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        if (mode === 'main') {
          // Trophy Mode: Hide large standalone open events
  ...
  const validateStep = (currentStep) => {
    ...
    if (currentStep === 1) {
      if (!college || !faculty || !leaderName || !leaderEmail || !leaderPhone) {
        return toast.error("Complete institutional profile to proceed.");
      }
      if (!validateEmail(leaderEmail)) return toast.error("Invalid Command Email.");
      if (!validatePhone(leaderPhone)) return toast.error("Mobile Uplink must be 10 digits.");

      if (mode !== 'main') return setStep(4);
      setStep(2);
    } else if (currentStep === 2) {
      if (teamInfo.selectedTrophyEvents.length === 0)
        return toast.error("Select at least one sector for the trophy hunt.");
      setStep(4);
    }
    window.scrollTo(0, 0);
  };
  ...
  const finalizeRegistration = async () => {
    ...
    for (let id of allSelectedIds) {
      ...
      if (!members || members.some(m => !m.name || !m.phone)) {
        return toast.error(`Incomplete operative data detected.`);
      }
      if (members.some(m => !validatePhone(m.phone))) {
        return toast.error("Operative contact must be 10 digits.");
      }
    }

    setLoading(true);
    try {
      // --- CRITICAL: HACKATHON COLLEGE LIMIT CHECK ---
      if (mode.toLowerCase() === 'hackathon') {
        const checkRes = await axios.get(`${baseURL}/api/admin/teams/check-limit`, {
          params: { college: teamInfo.college.trim(), event: 'Hackathon' }
        });
        
        if (checkRes.data.count >= 3) {
          setLoading(false);
          return toast.error(`🚨 ACCESS DENIED: ${teamInfo.college} has already deployed 3 teams for this event.`);
        }
      }
      ...
      const payload = {
        ...
        paymentStatus: 'pending', // All paths now require payment verification
      ...
      toast.success("Uplink Established! Synchronizing Payment...");

      // Redirecting everyone to the payment page for verification
      setTimeout(() => navigate(`/payment/${res.data.team._id}`), 1500);

    } catch (err) {
      toast.error(err.response?.data?.message || "Uplink Failed. Check system status.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-black min-vh-100 py-4 py-md-5 px-2 font-sans text-white">
      ...
      <div className="container-fluid max-w-4xl mx-auto px-1 px-md-3">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <Link to="/register" className="text-black text-decoration-none font-mono x-small border border-white border-opacity-20 px-3 py-2 rounded-pill bg-white bg-opacity-5 hover:text-cyan-400 transition-all">
            <i className="bi bi-arrow-left me-2"></i> RETURN_TO_HUB
          </Link>
          <h5 className="text-white font-mono mb-0 tracking-widest uppercase small">{mode}_Registration_System</h5>
        </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="st1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="card bg-glass border-white border-opacity-10 p-3 p-md-5 rounded-4">
              <h2 className="text-white font-black mb-1 uppercase tracking-tighter fs-3">Step 1: <span className="text-cyan-400">Institutional Intel</span></h2>
              <p className="text-white text-opacity-50 mb-4 small font-mono uppercase tracking-widest">Identify your unit</p>
              ...
              <div className="row g-3">
                ...
                <div className="col-12 col-md-6">
                  <label className="text-white x-small mb-1 d-block fw-bold tracking-widest">FACULTY ADVISOR</label>
                  <input type="text" className="form-control genesis-input" value={teamInfo.faculty} onChange={e => setTeamInfo({ ...teamInfo, faculty: e.target.value })} placeholder="Mentor Name" />
                </div>
                <div className="col-12 col-md-6">
                  <label className="text-white x-small mb-1 d-block fw-bold tracking-widest">COMMANDER (LEADER)</label>
                  <input type="text" className="form-control genesis-input" value={teamInfo.leaderName} onChange={e => setTeamInfo({ ...teamInfo, leaderName: e.target.value })} placeholder="Lead Student" />
                </div>
                <div className="col-12 col-md-6">
                  <label className="text-white x-small mb-1 d-block fw-bold tracking-widest">COMMS_EMAIL</label>
                ...
                <div className="col-12 col-md-6">
                  <label className="text-white x-small mb-1 d-block fw-bold tracking-widest">MOBILE_UPLINK</label>
                  <input type="text" maxLength="10" className="form-control genesis-input" value={teamInfo.leaderPhone} onChange={e => setTeamInfo({ ...teamInfo, leaderPhone: e.target.value.replace(/\D/g, '') })} placeholder="10 Digits" />
                </div>
              </div>
          ...
          {step === 2 && mode === 'main' && (
            <motion.div key="st2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="card bg-glass border-white border-opacity-10 p-3 p-md-5 rounded-4">
              <h2 className="text-white font-black mb-2 uppercase tracking-tighter fs-3">Step 2: <span className="text-cyan-400">Sector Selection</span></h2>
              <p className="text-white text-opacity-50 mb-4 small font-mono uppercase tracking-widest">Select Trophy Events</p>
                    ...
                    <div key={ev._id} className="col-12 col-sm-6">
                      <div className={`event-selector-v2 ${isSelected ? 'selected' : ''}`} onClick={() => handleEventToggle(ev, 'trophy')}>
                        ...
                        <div className={`check-icon ${isSelected ? 'checked' : ''}`}>
                          {isSelected && <i className="bi bi-check-lg text-black"></i>}
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="d-flex gap-3">
                <button className="btn-genesis-outline-v2 w-50" onClick={() => setStep(1)}>REVISE INFO</button>
                <button className="btn-genesis-v2 w-50" onClick={() => validateStep(2)}>CONFIGURE ROSTER</button>
              </div>
            </motion.div>
          )}
          ...
          {step === 4 && (
            <motion.div key="st4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card bg-glass border-white border-opacity-10 p-3 p-md-5 rounded-4 shadow-2xl">
              <h2 className="text-white font-black mb-1 uppercase tracking-tighter fs-3">Step 3: <span className="text-cyan-400">Unit Roster</span></h2>
              <p className="text-white text-opacity-50 mb-4 small font-mono uppercase tracking-widest">Input operative details</p>

              {events.filter(e => [...teamInfo.selectedTrophyEvents, ...teamInfo.selectedOpenEvents].includes(e._id)).map(event => (
                <div key={event._id} className="mb-5 p-3 p-md-4 rounded-4 bg-black bg-opacity-20 border border-white border-opacity-5 shadow-inner">
                  <div className="d-flex justify-content-between align-items-center border-bottom border-white border-opacity-10 pb-3 mb-4">
                    <h5 className="text-white font-mono mb-0 uppercase tracking-widest fw-bold">
                      <i className="bi bi-cpu text-cyan-400 me-2"></i>{event.name}
                    </h5>
                  ...
                  <div className="member-entries">
                      ...
                      <div key={idx} className="mb-4 pb-4 border-bottom border-white border-opacity-5">
                        <div className="row g-3">
                          <div className="col-12 col-md-5">
                            <label className="x-small-label mb-2 uppercase text-info tracking-widest">Operative Name</label>
                            <input type="text" className="form-control genesis-input-v2" value={member.name} placeholder="Identification" onChange={e => updateMember(event._id, idx, 'name', e.target.value)} />
                          </div>
                          <div className="col-7 col-md-4">
                            <label className="x-small-label mb-2 uppercase text-info tracking-widest">Mobile Link</label>
                          ...
                          {event.name.toLowerCase() !== 'football' && (
                            <div className="col-5 col-md-3">
                              <label className="x-small-label mb-2 uppercase text-white opacity-50 tracking-widest">Ration</label>
                        ...
                        {eventParticipants[event._id].length > event.minParticipants && (
                          <div className="text-end mt-2">
                            <button className="btn btn-link text-danger text-decoration-none x-small fw-bold p-0 font-mono" onClick={() => removeMemberRow(event._id, idx)}>
                              [TERMINATE_ROW]
                            </button>
                          </div>
                        )}
                    ...
                    {eventParticipants[event._id]?.length < event.maxParticipants && (
                      <button className="btn btn-outline-cyan-v2 btn-sm font-mono w-100 py-3 mt-2 tracking-widest uppercase" onClick={() => addMemberRow(event)}>
                        + RECRUIT_OPERATIVE
                      </button>
                    )}
                  </div>
              ...
              <div className="d-flex gap-3 mt-4">
                <button className="btn-genesis-outline-v2 w-50" onClick={() => mode === 'main' ? setStep(2) : setStep(1)}>REVISE SECTOR</button>
                <button className="btn-genesis-v2 w-50 shadow-cyan" onClick={finalizeRegistration} disabled={loading}>
                  {loading ? 'RUNNING_INTEGRITY_CHECK...' : 'FINALIZE_UPLINK'}
                </button>
              </div>
            </motion.div>
          )}
      ...
      <style>{`
        ...
        .genesis-input-v2, .genesis-input { background: rgba(0, 0, 0, 0.4) !important; border: 1px solid rgba(255, 255, 255, 0.1) !important; color: white !important; border-radius: 12px !important; padding: 12px !important; transition: 0.3s; }
        .genesis-input-v2:focus, .genesis-input:focus { border-color: #0dcaf0 !important; box-shadow: 0 0 15px rgba(13, 202, 240, 0.2) !important; outline: none; }
        .x-small-label { font-size: 0.6rem; font-weight: 800; color: #0dcaf0 !important; letter-spacing: 1px; }
        ...
        .btn-genesis-v2 { background: #0dcaf0; color: black !important; font-weight: 900; border: none; padding: 15px; border-radius: 12px; transition: 0.3s; cursor: pointer; text-transform: uppercase; letter-spacing: 1px; }
        .btn-genesis-v2:hover:not(:disabled) { transform: translateY(-2px); background: #00bacf; box-shadow: 0 5px 15px rgba(13, 202, 240, 0.3); }
        .btn-genesis-outline-v2 { background: transparent; color: white !important; border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 12px; padding: 15px; cursor: pointer; text-transform: uppercase; font-weight: bold; }
        .event-selector-v2 { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); padding: 15px; border-radius: 12px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; transition: 0.2s; }
        .event-selector-v2:hover { border-color: rgba(13, 202, 240, 0.4); }
        ...
        .select-arrow { background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'%3e%3cpath fill='none' stroke='%230dcaf0' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='m2 5 6 6 6-6'/%3e%3c/svg%3e"); background-repeat: no-repeat; background-position: right 0.75rem center; background-size: 16px 12px; appearance: none; }
        .btn-outline-cyan-v2 { color: #0dcaf0; border: 1px dashed #0dcaf0; background: transparent; padding: 10px; border-radius: 10px; cursor: pointer; transition: 0.2s; }
        .btn-outline-cyan-v2:hover { background: rgba(13, 202, 240, 0.05); border-style: solid; }
      `}</style>
    </div>
  );
};
